...
激光驱动 D-D 脉冲中子源 -> 外部锂靶产氚特性的跨尺度模拟方案。当前仓库是**研究计划与实现规格版本**，用于明确科学问题、模块边界、数据契约、验证关卡和一个月小论文执行路线；代码实现将在后续按规格补齐。
...
```text
[A] EPOCH PIC          [B] 半解析源项 Python           [C] OpenMC 输运
激光打 3-5 um CD2  ->  厚靶 D-D 产额 + 两体运动学  ->  锂靶 TPR
...
说明：当前 Stage B 使用占位 D-in-CD2 阻止本领，只用于软件链路验证。正式产额和论文结果前必须替换为 SRIM/PSTAR 或其他可靠数据。
