...
## 2026-07-03 claude (expert) - 策略重定位，推翻 3D-first

### 改动内容
- 新增 `docs/STRATEGY_REVISION_20260703_expert.md`，作为当前统筹口径，**取代**继承文档中"高精度 3D PIC 为可信度核心"的路线。
- 核心修订：可信度主干改为"验证过的中子学管线 + 参数化畸变稳健性扫描"；PIC 降为便宜锚点（2D 足够，3D 可选一发，不扫不追收敛）。
- PIC 源定义修订：弃 `rear+20µm / 5ps / 末窗<10%`，改为近面（rear+3~5µm）+ 短时（~1ps，E_max 与 >2.82MeV 占比 plateau 即停）+ 高 D PPC（8/32/4）。物理依据：产 >2.82MeV 中子的高能氘最早形成，几百 fs 抓全；低能宽角尾主要喂对源钝感的 ⁶Li。
- 明确论文"so what"：给紧凑/激光 D-D 源做增殖试验台的社区一个"理想源 vs 真实畸变源"的 TPR 修正量。
- 列出三个必补硬洞：CM 各向异性、SRIM/PSTAR 阻止本领、多库不确定度。

### 给后续 agent 的注意
- 优先级：Stage B→C 端到端 + 参数化主曲线 > 补三硬洞 > 便宜 3D 锚点。
- 便宜 3D 锚点用 C 变体（nx,ny,nz=1500,320,320，拓扑 16,4,4，restart 关），先 300fs 微基准实测再定正式那发。
- 不要再把机时投入高精度 3D 大扫描或 5ps rear+20µm 完整性追赶。
